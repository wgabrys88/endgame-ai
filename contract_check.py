"""Static organism contract validation for endgame-ai.

This is the immune-system canary for self-modification. It intentionally uses
only the standard library and AST parsing so it can run even when Windows UIA
dependencies are unavailable. It validates that a patch did not amputate the
organism's core organs while still producing syntactically valid Python.
"""
