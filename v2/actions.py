...
def _spawn_agent(args: dict, book: dict, state) -> ActionResult:
    ...
    cycles = str(args.get("cycles", "8"))
    cmd = ["python", str(BASE_DIR / "main.py"), goal, "--cycles", cycles, "--backend", get_backend()]
...
@_register("cleanup")
def _cleanup(args: dict, book: dict, state) -> ActionResult:
    import shutil
    ...
    if not runs_dir.exists():
        return ActionResult("cleanup", True, "no runs dir")
    all_runs = sorted([d for d in runs_dir.iterdir() if d.is_dir()], key=lambda p: p.stat().st_mtime, reverse=True)
    keep_names = set()
    for i, d in enumerate(all_runs):
        if i < 3 or "distill" in d.name.lower():
    ...
    cleaned_files = 0
    if comms.exists():
        for f in list(comms.glob("*_status.json")):
            try:
                f.unlink()
                cleaned_files += 1
            except Exception:
                pass
    summary = f"cleanup: kept={kept} removed={removed} comms_cleaned={cleaned_files}"
    return ActionResult("cleanup", True, summary)
