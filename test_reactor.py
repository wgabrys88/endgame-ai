...
def _report_paths(model: str) -> tuple[Path, Path]:
    ...
    report = BASE / "runtime" / "comms" / f"test_report{suffix}.md"
    summary = BASE / "runtime" / "comms" / f"test_summary{suffix}.json"
...
def main() -> int:
    ...
    print(f"[test_reactor] Cleaning runtime and starting 2-slot localhost test for {duration:.0f}s (model={model or 'default'})")
    ...
    stdout_log = BASE / "runtime" / "comms" / f"test_reactor_stdout{('_' + model) if model else ''}.txt"
