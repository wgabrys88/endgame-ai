...
class Reflect(LlmNode):
    ...
    def payload(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        return {'goal': goal, 'goal_narration': state.get('goal_narration', goal), 'step': {'description': step.get('description', goal), 'done_when': step.get('done_when', '')}, 'observation': bus.observation_brief(state), 'action_index': bus.action_index_brief(state), 'action_frame': state.get('action_frame'), 'evidence': {'last_action': state.get('last_action', {}), 'last_result': state.get('last_result', ''), 'last_error': state.get('last_error', ''), 'last_verification': state.get('last_verification', {}), 'failure_streak': streak_patch['failure_streak'], 'state': bus.state_brief(state)}, 'routing_rule': {'retry': 'observe rescan then execute — try click_node(id) or new frame_action target', 'replan': 'plan step wrong; planner must rewrite intent with observable SEMANTIC_UI roles', 'escalate': 'firmware/prompt/observation contract broken; self_modify patches Python', 'give_up': 'goal impossible for this body'}}
