...
## The division paradox (this branch's story)

This section is true of `runner-zebra` and is the reason this README now lives *inside* the organism.

For a day we believed the body was too large and split its heavy Windows machinery (the ctypes/COM/UI-Automation eyes and the mouse-and-key hand, roughly eight hundred lines) out into a sibling file `win32.py`, wiring it back in as a fifth, self-healable body-part. It worked; a real dry run proved the moved perception path still saw the live desktop. But the split taught us the opposite of what we set out to prove.

Two truths surfaced. First, the size was never the problem: the organism edits itself one section at a time and rarely reads its whole body, and any mind that runs it can read the whole file at once - eight hundred lines of device plumbing is no burden. Second, and deeper: main's README claimed *"you are reading the organism's DNA"* while sitting in a separate file - a small lie, told by the very document that preaches one-source-of-truth. The real defect was not that the body was big; it was that the body was **not actually whole**.

So we reversed. `endgame.md` returned to the exact single-file body proven on main, `win32.py` was deleted, and this README moved *into* the file as the `## readme` section you are reading now. We set out to divide the organism and instead unified it more completely. That is the paradox, and it is the win. The split remains reachable in git history for any future that genuinely needs a two-file form; it is not the road we chose. The moment on the branch where the single-file body was restored is marked by the tag `division-paradox`.

One elegant idea was raised and deliberately set aside: since the organism already fetches text from the web each turn and always has an internet-backed mind, the hand could be *downloaded* on first need rather than shipped in the body - a console-born organism that acquires its own eyes and hand from a URL only if it chooses to. It is a beautiful shape and it is recorded here on purpose. It was rejected for one hard reason: the organism can also run against a **local** mind (LM Studio) with no internet, so a hand that depends on a download would be a hand that sometimes cannot exist. A body-part must not be a live remote dependency. The idea waits in these words for a future that may want it.

## What runner-zebra changed from main

This branch is the single-file body of main tip, byte-for-byte, plus exactly two small engine fixes and this readme section. Nothing else differs. Stated plainly so any future reader can trust the diff:

- **Per-run transmission folders.** The audit dumps of every model call now land in `.transmissions/<YYYY-MM-DD-HH-MM-SS>/`, a folder stamped once when the process starts, instead of one flat directory. The stamp is exported to child deed processes through the environment variable `EGAI_RUN_DIR`, so a whole run - parent turns and spawned deeds alike - files itself into one folder. Runs self-separate for archiving. If transmission logging is off, nothing changes.

- **Correct web_search parsing.** The web-search helper used to read the answer text only from response items whose type was exactly `message`; any other non-reasoning shape returned empty and raised a false failure. It now reads text from every output item that is not a reasoning item (the same rule the engine already uses elsewhere), collects citation URLs whether they arrive as annotations or as a top-level list of strings or objects, and still fails hard when there is genuinely no text. It does **not** disable reasoning, does **not** double-count the answer, and does **not** pad the query with instructions - three changes that were considered and rejected as regressions.

Both fixes were verified offline: the config parses, the engine, reset, and capabilities sections compile, and the per-run folder logic was unit-tested.

## Running the tests

You supply a one-sentence goal; the organism does the rest. On the GUI host, with `XAI_API_KEY` set, set the goal into the `## goal` slot and turn the wheel. A compact way to set-goal-and-launch in one line (the file is `endgame.md`):

```bat
python -c "import re,pathlib,sys; g=sys.argv[1]; p=pathlib.Path('endgame.md'); t=re.sub('## goal\n.*?(?=\n## )','## goal\n'+g+'\n',p.read_text(encoding='utf-8'),flags=re.S); p.write_text(t,encoding='utf-8'); f=chr(96)*3; exec(re.search(r'##\s+engine\s*\n'+f+'python\n(.*?)^'+f, t, re.S|re.M).group(1), {'BOARD':str(p),'ARGV':[sys.argv[0],str(p)]})" "YOUR ONE-SENTENCE GOAL"
```

To hold a run to a fixed wall-clock and then stop it cleanly, launch it as a subprocess and tree-kill after the limit (the organism spawns child deed processes, so a plain kill is not enough):

```powershell
$p = Start-Process python -PassThru -ArgumentList '-c', '<the one-liner above>', 'YOUR ONE-SENTENCE GOAL'
if (-not $p.WaitForExit(600000)) { taskkill /PID $p.Id /T /F }   # 600000 ms = 10 minutes
```

Between runs, clear memory and goal back to seed (learned nodes survive):

```bash
python endgame.md --reset
```

After each run, the run's folder under `.transmissions/` is a self-contained record to archive.

## Three proving runs

Three goals that together exercise the whole surface - the GUI hand, live web search, the learning node graph, and parallel spawning - each provable by a handless witness reading a file, and each sized to finish inside a generous hour. Each goal also asks the organism to keep an open Notepad window as a running, human-readable journal of what it is doing and why. That journal is the organism's *own testimony* - commentary, not proof; the witness still proves the real result by the files on disk - but it lets a watching human read the organism's thought and see parallelism as overlapping narration, a living log spoken by the system itself.

1. **Hand and web, narrated.** *Keep a Notepad window open on this desktop as your running journal, and each time you act write one plain-English line into it saying what you just did and why; using that journal as your visible log, look up from the live web the current stable Python version and today's weather in Warsaw, and save both facts with their source URLs to a file on the Desktop.* Done when that file holds the two facts and their two source URLs, and the journal shows the step-by-step narration. Stresses the hand, `web_search`, and multi-step chaining.

2. **Learned repetition, narrated.** *Keeping the same open Notepad journal updated after every step, create ten numbered text files in a Desktop folder, each holding its own number, and find a reusable way to do it so the same proven method carries the repetition rather than being written fresh each time.* Done when all ten files exist with correct contents and the journal shows one method being reused across the ten. Stresses `save_node` / `call_node`, edge reinforcement on witnessed confirms, and stigmergic routing.

3. **Parallel work, made visible.** *With your Notepad journal open and updated throughout, build a JSON file on the Desktop mapping five African countries to their capitals researched from the live web, dividing the lookups so several proceed at once, and have each parallel strand announce itself in the journal as it works.* Done when the JSON is valid with five correct pairs and the journal visibly shows more than one strand working at once. Stresses `spawn_actor` and web research together.

The point of the Notepad journal across all three is a second, human channel of truth running beside the machine logs: you watch the organism explain itself, in its own words, as it lives.
