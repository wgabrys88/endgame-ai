...
BOARD = globals().get("BOARD", "endgame-ai-live.md")
...
# The deed runs as its own program, not in-process. Whatever the prompt promises the
# model by bare name (desktop, action_index, ...) is glued onto deed.py so the promise
# is kept at execution. The witness gets no `desktop`: the Law of Separated Powers holds.
_DEED_PREAMBLE = """import json as _json, sys as _sys, pathlib as _pl
_ctx = _json.loads(_pl.Path(_sys.argv[1]).read_text(encoding="utf-8"))
action_index = _ctx.get("action_index", {})
screen_elements = _ctx.get("screen_elements", [])
desktop_tree_text = _ctx.get("desktop_tree_text", "")
repo_root = _ctx.get("repo_root", ".")
python_executable = _ctx.get("python_executable", _sys.executable)
signal = None
verdict = None
if _ctx.get("kind") == "actor":
    import capabilities as _caps, types as _types
    _d = _caps.get_desktop()
    desktop = _types.SimpleNamespace(click=_d.click, type_text=_d.type_text,
        paste_clipboard=_d.paste_clipboard, set_clipboard=_d.set_clipboard,
        press_key=_d.press_key, hotkey=_d.hotkey, scroll=_d.scroll, open_url=_d.open_url)
# ===== deed follows =====
"""

_DEED_EPILOGUE = """
# ===== deed done: post signal + verdict back to the result file =====
_pl.Path(_sys.argv[2]).write_text(_json.dumps(
    {"signal": (str(signal) if signal is not None else "ok"), "verdict": verdict},
    default=str), encoding="utf-8")
"""


def _materialize_capabilities(here):
    """Write the `## capabilities` block to a real capabilities.py so deed.py can import it."""
    sections, _order = read_board(BOARD)
    m = re.search(r"```(?:python)?\s*(.*?)```", sections.get("capabilities", ""), re.S)
    if m and m.group(1).strip():
        (here / "capabilities.py").write_text(m.group(1).strip() + "\n", encoding="utf-8")


def run_exec(code, ns_kind, sections):
    here = pathlib.Path(BOARD).resolve().parent
    _materialize_capabilities(here)
    c = caps()
    obs = getattr(c, "_LAST_OBS", {}) if c is not None else {}
    ctx = {
        "kind": ns_kind,
    ...
    ctx_path = here / "_deed_ctx.json"
    res_path = here / "_deed_result.json"
    deed_path = here / "deed.py"
    ctx_path.write_text(json.dumps(ctx, default=str), encoding="utf-8")
    if res_path.exists():
        res_path.unlink()
    deed_path.write_text(_DEED_PREAMBLE + code + _DEED_EPILOGUE, encoding="utf-8")
    proc = subprocess.run([sys.executable, str(deed_path), str(ctx_path), str(res_path)],
                          capture_output=True, text=True, cwd=str(here))
    if res_path.exists():
        r = json.loads(res_path.read_text(encoding="utf-8"))
        sig = str(r.get("signal") or "ok")
        verdict = r.get("verdict")
        out = proc.stdout or ""
        if verdict is not None:
            out = json.dumps(verdict, default=str) + ("\n" + out if out else "")
        return sig, (out.strip() or "(no output)")
    # no result file => the deed raised before posting; that is a fault, shown loud
    return "fault", (proc.stderr or proc.stdout or "deed produced no result").strip()
