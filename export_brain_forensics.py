...
def _escape_string_content(text: str) -> str:
    """Escape one line of JSON string content without surrounding quotes."""
    parts: list[str] = []
    for char in text:
        if char == '"':
            parts.append('\\"')
...
def _append_multiline_string_entry(
    ...
    """Emit multiline strings with content indented under their key."""
...
def human_json_dumps(value: Any, indent: int = 0, step: int = 2) -> str:
    """Pretty-print JSON with literal newlines inside multiline strings."""
    pad = " " * indent
...
def split_xai_console_record(record: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    """Preserve the full console row; split request vs response under logged.chat."""
    ...
    request = {key: value for key, value in record.items() if key not in {"logged"}}
    response = {key: value for key, value in record.items() if key not in {"logged"}}
    request["logged"] = {
        **other_logged,
