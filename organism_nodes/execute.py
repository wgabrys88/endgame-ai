...
import io
import nodes
DATASHEET = bus.datasheet('execute', kind='llm_code_actuator', inputs=['goal', 'current_step', 'fresh_observation', 'action_frame', 'capability_runtime'], signals=['verify', 'frame', 'reflect', 'self_modify', 'error'], writes=['last_action', 'last_code', 'last_result', 'last_error', 'action_frame'], record_type='execution')
...
def run(ctx):
    ...
    payload = {'goal': goal, 'step': {'description': step.get('description', goal), 'done_when': step.get('done_when', '')}, 'state': bus.state_brief(state), 'observation': bus.observation_brief(state), 'action_frame': state.get('action_frame'), 'last': {'error': state.get('last_error'), 'result': state.get('last_result', ''), 'action': state.get('last_action', {})}, 'capability_contract': {'body_truth': 'Code runs locally in the organism capability runtime. The listed helpers are conveniences, not the full limit of Python.', 'pyautogui_compat': 'Use pyautogui.click/write/press/hotkey/scroll naturally; it is a dependency-free facade backed by the organism body.', 'helpers': ['click_node(id)', 'scroll_node(id, amount)', 'node_by_id(id)', 'action_nodes(action=None)', 'click(x,y,hwnd=0)', 'type_text(text)', 'press_key(key)', 'hotkey(keys)', 'scroll(x,y,amount,hwnd=0)', 'focus_window(target)', 'open_url(browser,url)'], 'modules': ['subprocess', 'ctypes', 'os', 'sys', 'json', 're', 'time', 'pathlib', 'math', 'random'], 'result_rule': 'Set result to a small JSON-serializable value describing what happened.'}}
