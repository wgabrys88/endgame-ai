...
def _call_api(messages, cfg):
    ...
    payload = {'model': model, 'input': input_data, 'temperature': cfg.get('temperature', 0.2), 'truncation': str(cfg.get('truncation') or 'disabled')}
    ...
    if isinstance(response_format, dict):
        ...
        if fmt_type == 'json_object':
            ...
            payload['text'] = {'format': {'type': fmt_type, 'name': response_format.get('name', 'record'), 'schema': response_format.get('schema', {}), 'strict': bool(response_format.get('strict', True))}}
